...
int main()
{
   ...
   // Criar o bloco de memória partilhada
   ...
   // Criar os pipes
   int fd_consumer[2], fd_producer[2];

   if (pipe(fd_consumer) == -1 || pipe(fd_producer) == -1)
   {
   ...
   else if (pid == 0)
   {
      ...
      for (i = 0; i < NR_EXCHANGED; i++)
      {
         ...
         // Depois da leitura, escrever no pipe
         write(fd_consumer[1], &i, sizeof(int));
      }
      exit(EXIT_SUCCESS);
   }
   ...
   {
      // Escrever no pipe
      write(fd_producer[1], &i, sizeof(int));

      // Esperar que o reader leia
      ...
      // Depois da escrita, ler do pipe
      read(fd_consumer[0], &i, sizeof(int));
   }

   // Apagar o bloco de memória partilhada
